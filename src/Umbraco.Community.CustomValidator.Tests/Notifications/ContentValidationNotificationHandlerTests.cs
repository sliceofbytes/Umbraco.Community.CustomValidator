...
public sealed class ContentValidationNotificationHandlerTests
{
    ...
    public async Task HandleAsync_ContentPublishing_ExceptionThrown_CancelsPublish()
    {
        ...
        // The corrected handler includes the exception type name in the user-facing message
        // for easier diagnosis without diving into logs.
    ...
    {
        // Arrange
        // The corrected handler explicitly rethrows OperationCanceledException rather than
        // masking it as a generic validation failure. This protects cooperative cancellation
        // semantics for callers in background-job contexts (Hangfire, MassTransit, etc.).
        _umbracoContextFactoryMock.Setup(x => x.EnsureUmbracoContext())
    ...
    /// <see cref="IUmbracoContext"/>. <c>isRoot: false</c> ensures <c>Dispose()</c> is a
    /// no-op — we don't want the reference to try to dispose the underlying mock context
    /// or clear the accessor when the SUT's <c>using</c> block exits.
    /// </summary>
